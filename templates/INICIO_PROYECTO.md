...
9. El informe final de la TASK (Fase L) es lo que se resume en `PROGRESS.md` como checkpoint de contexto — no el razonamiento intermedio. El checkpoint de una fase F cerrada debe incluir la **evidencia cruda** de validación (bloque de código con la salida de tests/build, o `Evidencia: <archivo>`): `task_generator.py --check` lo exige. Además, antes de cerrar, emite el **recibo de fase** (`fia receipt create F<N> --tests P/T [--evidence EV-NNN]`) y añade `Recibo: evidence/receipts/receipt-F<N>.json` al checkpoint (regla nº16).
...
16. **Ninguna fase F se cierra sin su recibo verificable (v3.3).** Emite el recibo con `fia receipt create F<N> --tests P/T [--evidence EV-NNN]` **antes** de cerrar y añade `Recibo: evidence/receipts/receipt-F<N>.json` a su checkpoint. `--check` y el CI lo exigen; el recibo ata el contenido de los archivos al commit y detecta manipulación posterior.

> 🚨 **Enforcement:** cuando el repositorio está en GitHub con el workflow emitido por `bootstrap.py` (`.github/workflows/harness.yml`), se verifican además mecánicamente en cada push/PR las reglas **4, 5, 7 y 16**: `task_generator.py --check` valida el estado (`progress.json` sincronizado, dependencias cerradas en orden, checkpoint y `TASK-Fx.md` por cada fase cerrada, aprobaciones `APPROVAL-NNN` citadas registradas, recibo de fase verificado), gitleaks escanea secretos y la auditoría de dependencias corre según el stack detectado. Las reglas **9 y 13** se refuerzan con el gate de riesgo/Lite (decisión humana registrada). Las reglas **8 (AEO)** y **12 (Design DNA)** aún no tienen gate mecánico (la 12 está diferida a v3.5, ADR-011). Un push que viole una regla no pasa el merge.
