...
> 🚨 **Enforcement:** cuando el repositorio está en GitHub con el workflow emitido por `bootstrap.py` (`.github/workflows/harness.yml`), las reglas 4, 5, 7, 8 y 16 se verifican además mecánicamente en cada push/PR: `task_generator.py --check` valida el estado (`progress.json` sincronizado, dependencias cerradas en orden, checkpoint y `TASK-Fx.md` por cada fase cerrada, aprobaciones `APPROVAL-NNN` citadas registradas, recibo de fase verificado), gitleaks escanea secretos y la auditoría de dependencias corre según el stack detectado. Un push que viole una regla no pasa el merge.
